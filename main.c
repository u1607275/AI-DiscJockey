...
int main(int argc, char *argv[]){
	...
	if(argc != 3){
		g_print("Two parameters needed (pitch change in semitones, time stretch as decimal)");
		return -1;
	}

	float pitchShiftExp = atof(argv[1])/12;
	...
	g_object_set( G_OBJECT(data.timePitch), "pitch", pow(2,pitchShiftExp), "tempo", timeStretch, NULL);
